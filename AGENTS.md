...
**Operating units / branch scope:** `Organization` owns `operating_units`; seeds create institution **`BANKCORE`** and branch **`MAIN`**. Operators carry `default_operating_unit_id`, teller sessions carry `operating_unit_id`, and staff-originated operational events on `teller` / `branch` channels carry `operating_unit_id` where scope can be resolved. Existing Teller JSON clients do **not** need to send `operating_unit_id` while there is one active branch; scope resolves from teller session, explicit authorized selection, operator default, then the seeded branch. Scoped RBAC uses exact `operator_role_assignments.scope_type = "operating_unit"` / `scope_id = operating_units.id` plus global assignments; no region/parent inheritance is implied. See [docs/adr/0032-operating-units-and-branch-scope.md](docs/adr/0032-operating-units-and-branch-scope.md).
...
**Holds:** **`POST /teller/holds`** (JSON `hold` object) creates an **`active`** hold and a posted **`hold.placed`** event. Optional **`placed_for_operational_event_id`** (integer) links the hold to a **posted** **`deposit.accepted`** or **`check.deposit.accepted`** on the same **`deposit_account_id`**; the sum of **active** holds on that deposit cannot exceed the deposit amount (ADR-0013 §3). **`POST /teller/holds/release`** releases a hold (**`hold_release`** payload). Reversing a deposit or check deposit via **`POST /teller/reversals`** is rejected while any **active** hold still references that operational event id.

**Check deposits (T1):** **`POST /teller/operational_events`** with **`event_type: "check.deposit.accepted"`** runs **`AcceptCheckDeposit`**: **`RecordEvent` → `PostEvent` → optional hold** in **one** server transaction (`channel` must be **`teller`**). Request body uses the usual scalars plus nested **`payload`**: **`{ "items": [ { "amount_minor_units", exactly one of "item_reference" or "serial_number", optional "classification": "on_us"|"transit"|"unknown" } ] }`**; **`amount_minor_units`** on the event must equal the sum of item amounts ([docs/operational_events/check-deposit-accepted.md](docs/operational_events/check-deposit-accepted.md)). Optional top-level **`hold_amount_minor_units`** and **`hold_idempotency_key`** (must **differ** from the deposit **`idempotency_key`**) place an **event-level** hold at acceptance; optional **`hold_expires_on`** (ISO **`YYYY-MM-DD`**) sets the hold’s **`expires_on`** (scheduled release date; must be on or after the acceptance business date). Successful responses include **`posting_outcome`** (**`posted`** / **`already_posted`** on replay) and optional **`hold_id`** / **`hold_outcome`**. **`GET /teller/operational_events`** returns **`payload_summary`** for this type only (not full **`items`**). Example:

```bash
curl -sS -X POST "http://localhost:3000/teller/operational_events" \
  -H "Content-Type: application/json" -H "X-Operator-Id: YOUR_TELLER_OPERATOR_ID" \
  -d '{"operational_event":{"event_type":"check.deposit.accepted","channel":"teller","idempotency_key":"chk-demo-1","amount_minor_units":500,"currency":"USD","source_account_id":YOUR_ACCOUNT_ID,"payload":{"items":[{"amount_minor_units":500,"item_reference":"MICR-DEMO"}]},"hold_amount_minor_units":500,"hold_idempotency_key":"chk-demo-1-hold","hold_expires_on":"2026-05-15"}}'
```
...
**Statements:** deposit statement configuration is product-owned in `deposit_product_statement_profiles`; generated snapshots live in `deposit_statements` under the `Deposits` domain. `Deposits::Commands::GenerateDepositStatements` is domain-only in P3-5 and derives line items from posted GL **2110** journal lines plus selected no-GL servicing events (`hold.placed`, `hold.released`, `overdraft.nsf_denied`). No teller/customer HTTP route or document delivery is included in this slice (ADR-0024).

**Internal HTML workspaces:** Phase 3.5 adds Rails-rendered internal workspaces under **`branch`**, **`ops`**, and **`admin`**. `branch` initially wraps teller workflows but intentionally leaves existing JSON **`/teller`** routes and **`X-Operator-Id`** behavior unchanged. Browser auth is staff/operator session based and internal-only; do not reuse it for customer or partner channels (ADR-0025).

**Branch CSR servicing:** Phase 4.1 extends **`branch`** with customer/account servicing. Use **`GET /branch/customers`** to search, **`GET /branch/customers/:id`** for customer 360, and **`GET /branch/deposit_accounts/:id`** for account profile, balances, holds, statement metadata, recent activity, and operational-event traceability. Branch servicing writes use operational-event channel **`branch`** with `actor_id`; teller cash still uses channel **`teller`** and teller-session rules. A **supervisor** is required for Branch fee waivers, hold release, and reversals. See [docs/adr/0026-branch-csr-servicing.md](docs/adr/0026-branch-csr-servicing.md).

**Operational events (observability):** **`GET /teller/operational_events`** and the Ops event search use `Core::OperationalEvents::Queries::ListOperationalEvents` — query params: optional **`business_date`** (single day) **or** **`business_date_from`** + **`business_date_to`** (inclusive, max 31 days); optional **`source_account_id`**, **`destination_account_id`**, **`status`**, **`event_type`**, **`channel`**, **`actor_id`**, support keys **`reference_id`**, **`idempotency_key`**, **`reversal_of_event_id`**, plus **`deposit_product_id`**, **`product_code`**; pagination **`limit`** (default 50, max 200) and **`after_id`**. No date after **`current_business_on`**. Response includes **`current_business_on`**, **`posting_day_closed`**, **`business_date_from`**, **`business_date_to`**, **`next_after_id`**, **`has_more`**, and **`events`** (each with **`operating_unit_id`**, **`source_account`** / **`destination_account`** product summary when linked, plus **`posting_batch_ids`** and **`journal_entry_ids`**). See [docs/adr/0017-deposit-products-fk-narrow-scope.md](docs/adr/0017-deposit-products-fk-narrow-scope.md) §2.5.

**ACH receipt ingestion:** `Integration::Ach::Commands::IngestReceiptFile` is the narrow Phase 4.7 inbound ACH credit path. It accepts structured file/batch/item input, preserves account-number leading zeroes, resolves accounts through `Accounts::Queries::FindDepositAccountByAccountNumber`, records/posts **`ach.credit.received`** on channel **`batch`**, and posts Dr **1120** / Cr **2110** through `Core::Posting`. Ops staff can upload or paste this structured JSON through the **ACH receipt ingestion** screen; `docs/samples/ach-receipt-sample.json` is a replaceable sample payload. Item idempotency is deterministic: **`ach-credit-received:{file_id}:{batch_id}:{item_id}`**; support reference is **`ach:{file_id}:{batch_id}:{item_id}`**. Use Ops event search `reference_id` / `idempotency_key` filters for support evidence. Do not add ACH debits, origination, returns/NOCs, full NACHA parsing, cutoff warehousing, persisted file/batch lifecycle tables, or receiver-name matching without follow-up ADR coverage. See [docs/adr/0028-ach-receipt-ingestion.md](docs/adr/0028-ach-receipt-ingestion.md).
