...
| **4.5 Support observability and close readiness** | **Next recommended** | Add support search fields/indexes, read-only close evidence packages, and EOD impact visibility for channels. Avoid materialized balance snapshots or branch-scoped business dates until volume or branch identity requires them. |
...
**Current:** use **§4**, **§7**, **§8**, and **§10** as the live checkpoint (Phase 1 breadth, Phase 2 and Phase 3 narrow tracks, plus Phase 4.1-4.4 Branch servicing, event catalog, product resolver, and servicing-depth slices); when adding `event_type` values or crossing module boundaries, extend an integration proof per [.cursor/rules/bankcore-planning.mdc](../.cursor/rules/bankcore-planning.mdc).
