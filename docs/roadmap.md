...
- **Ops workspace:** **Close package** first (`/ops/close_package` — EOD readiness composition, ADR-0041 classification, trial balance, embedded business-date close); legacy **EOD readiness** and **business date close** screens retained; operational event search/detail and exception review.
- **Admin/Product workspace:** product configuration inspection first, then guarded config edits in later slices.

**Phase 3.5b refinement:** extend those workspaces toward parity with shipped Phase 0-3 operations: branch session lifecycle with expected-cash close display, transfer/hold/reversal/override/event screens; **ops Close package** as canonical EOD workspace ([ADR-0041](adr/0041-close-package-and-eod-classification-taxonomy.md) T4.1 classification under `Teller::Queries::ClosePackageClassification`) plus exception queues; admin product readiness and richer product detail. This remains an internal staff UI pass and does **not** introduce new event types, posting semantics, GL mappings, cash-location depth, or external channels.
...
| **4.5 Support observability and close readiness** | **Shipped** | Ops support search now includes support-key filters for reference, idempotency, and reversal links, with query-aligned operational-event indexes. The **Close package** consolidates EOD review with **derived classification** (primary buckets per [ADR-0041](adr/0041-close-package-and-eod-classification-taxonomy.md)), trial balance, and guarded close with redirects to `/ops/close_package`; raw breakdowns by status, channel, and event type remain for support. Materialized balance snapshots for close artifacts and branch-scoped business dates remain deferred where not already covered elsewhere. |
| **4.6 External read APIs** | **Remaining planned Phase 4 slice** | Add customer/partner/fintech read contracts over existing account, event, product, statement, and ledger-derived state. [ADR-0027](adr/0027-external-read-api-boundary.md) and the [external read API contract plan](external-read-api-contract-plan.md) define the trust boundary, redaction, rate-limit, audit, pagination, route, serializer, and response-contract requirements. Do not reuse Branch browser sessions or teller headers. |
| **4.7 First money-moving channel** | **Shipped** | Narrow ACH receipt ingestion is the first external money-moving slice. [ADR-0028](adr/0028-ach-receipt-ingestion.md) defines the accepted scope: structured file/batch/item input, deterministic item idempotency, exact account-number lookup, `ach.credit.received`, settlement GL **1120**, current-open-day posting, Ops HTML upload/paste ingestion, support search by reference/idempotency keys, per-item reconciliation evidence, and a sample JSON file in [`docs/samples/ach-receipt-sample.json`](samples/ach-receipt-sample.json). ACH origination, debits, returns/NOCs, cutoff warehousing, full NACHA parsing, and persisted file/batch lifecycle tables remain deferred. |
| **4.8 Operating-unit branch scope** | **Shipped** | [ADR-0032](adr/0032-operating-units-and-branch-scope.md) first slice introduces the `Organization`-owned `operating_units` table with seeded institution/branch records, operator defaults, teller-session scope, staff-originated operational-event `operating_unit_id`, and exact operating-unit scoped RBAC. Branch-scoped business dates, branch GL, account home-branch ownership, hierarchy inheritance, and Cash-domain locations remain deferred. |

Wires, cards/ATM, full multi-branch or multi-entity GL, document delivery, broad reporting snapshots, allowed-overdraft/representment, hierarchy-inherited operating-unit authorization, and dispute/provisional-credit workflows remain deferred unless the selected Phase 4 channel explicitly requires them.
...
**Current:** use **§4**, **§7**, **§8**, and **§10** as the live checkpoint (Phase 1 breadth, Phase 2 and Phase 3 narrow tracks, plus shipped Phase 4 Branch servicing, event catalog, product resolver, servicing depth, support observability, narrow ACH receipt ingestion, and operating-unit branch-scope slices). The only remaining planned Phase 4 slice is **4.6 External read APIs**; when adding `event_type` values or crossing module boundaries, extend an integration proof per [.cursor/rules/bankcore-planning.mdc](../.cursor/rules/bankcore-planning.mdc).
