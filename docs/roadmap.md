...
| **4.2 Event catalog and channel metadata** | **Next recommended** | Expand `EventCatalog` lifecycle/channel metadata, statement/customer visibility flags, payload schema references, and docs drift checks across the catalog, posting registry, and [operational event specs](operational_events/). This is the preferred next slice because ACH, external APIs, statements, and support search depend on reliable event semantics. |
| **4.3 Product resolver baseline** | Planned | Define resolver contracts and effective-dated helper conventions for product behavior. Defer full product-engine breadth; add per-product GL mapping only when the selected money-moving channel requires it. |
| **4.4 Servicing depth after CSR** | Planned | Extend account servicing with focused holds depth (expiration, reason/type taxonomy, partial release/adjustment only if required) and account-party read/write workflows after role/authority semantics are clear. |
| **4.5 Support observability and close readiness** | Planned | Add support search fields/indexes, read-only close evidence packages, and EOD impact visibility for channels. Avoid materialized balance snapshots or branch-scoped business dates until volume or branch identity requires them. |
| **4.6 External read APIs** | Planned | Add customer/partner/fintech read contracts over existing account, event, product, statement, and ledger-derived state. Requires an external API ADR covering auth, redaction, rate limits, idempotency expectations, audit attribution, and response contracts. Do not reuse Branch browser sessions or teller headers. |
...
**Current:** use **§4**, **§7**, **§8**, and **§10** as the live checkpoint (Phase 1 breadth, Phase 2 and Phase 3 narrow tracks, plus Phase 4.1 Branch CSR); when adding `event_type` values or crossing module boundaries, extend an integration proof per [.cursor/rules/bankcore-planning.mdc](../.cursor/rules/bankcore-planning.mdc).
