...
| **4.7 First money-moving channel** | **Shipped** | Narrow ACH receipt ingestion is the first external money-moving slice. [ADR-0028](adr/0028-ach-receipt-ingestion.md) defines the accepted scope: structured file/batch/item input, deterministic item idempotency, exact account-number lookup, `ach.credit.received`, settlement GL **1120**, current-open-day posting, support search by reference/idempotency keys, and per-item reconciliation evidence. ACH origination, debits, returns/NOCs, cutoff warehousing, full NACHA parsing, and file/batch persistence remain deferred. |
...
**Current:** use **§4**, **§7**, **§8**, and **§10** as the live checkpoint (Phase 1 breadth, Phase 2 and Phase 3 narrow tracks, plus shipped Phase 4 Branch servicing, event catalog, product resolver, servicing depth, support observability, and narrow ACH receipt ingestion slices); when adding `event_type` values or crossing module boundaries, extend an integration proof per [.cursor/rules/bankcore-planning.mdc](../.cursor/rules/bankcore-planning.mdc).
