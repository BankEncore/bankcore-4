...
**Current:** use **§4**, **§7**, **§8**, and **§10** as the live checkpoint (Phase 1 breadth, Phase 2 and Phase 3 narrow tracks, plus shipped Phase 4 Branch servicing, event catalog, product resolver, servicing depth, support observability, and narrow ACH receipt ingestion slices). The only remaining planned Phase 4 slice is **4.6 External read APIs**; when adding `event_type` values or crossing module boundaries, extend an integration proof per [.cursor/rules/bankcore-planning.mdc](../.cursor/rules/bankcore-planning.mdc).
