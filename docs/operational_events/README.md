...
| [check-deposit-accepted.md](check-deposit-accepted.md) | `check.deposit.accepted` | Yes | `AcceptCheckDeposit` | `pending_to_posted` | `teller` | Yes | Yes |
...
**Operating-unit attribution:** ADR-0032 adds `Organization`-owned `operating_units` and persists `operating_unit_id` on teller sessions and staff-originated operational events when scope can be resolved. Existing Teller JSON clients may omit `operating_unit_id` while one active branch exists; scope resolves from teller session, explicit authorized selection, operator default, then the seeded branch. `GET /teller/operational_events` includes `operating_unit_id` in each event row for support evidence. This field is operational context only and must not be used as branch-level GL or business-date truth.
